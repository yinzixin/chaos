...
### How to construct a splitting field: the general recipe, run in slow motion

The existence proof above is really an **algorithm** — a loop:

1. **Factor** $f$ into irreducible factors over your *current* field.
2. If all factors are linear — done: $f$ splits, and the field built so far is the splitting field.
3. Otherwise, **pick an irreducible factor of degree $\ge 2$ and adjoin one of its roots** (concretely: pick a root $\alpha \in \mathbb{C}$ and form $F(\alpha)$; abstractly: Kronecker's quotient $F[x]/\langle p\rangle$).
4. Return to step 1 — and here is the crucial point: **re-factor over the *new*, bigger field**, where $f$ may break up further than before.

Each pass peels off at least one more linear factor, so the loop terminates in at most $\deg f$ steps. Let us run it on $f(x) = x^3 - 2$ over $\mathbb{Q}$ (Example 11), where it genuinely needs two passes.

**Where the roots live.** In $\mathbb{C}$, the three cube roots of $2$ all have modulus $\sqrt[3]2$ and sit at angles $0°, 120°, 240°$:
$$\alpha_1 = \sqrt[3]2, \qquad \alpha_2 = \omega\sqrt[3]2, \qquad \alpha_3 = \omega^2\sqrt[3]2.$$
One root is real; the other two are a complex-conjugate pair.

**Pass 1.** Over $\mathbb{Q}$, $f$ is irreducible (no rational root). Adjoin the real root: $K_1 = \mathbb{Q}(\sqrt[3]2)$, with $[K_1 : \mathbb{Q}] = 3$.

**Pass 2 — re-factor over $K_1$.** Now that $\sqrt[3]2 \in K_1$, divide it out (division algorithm):
$$x^3 - 2 = (x - \sqrt[3]2)\,\big(x^2 + \sqrt[3]2\,x + \sqrt[3]4\big).$$
(Check by expanding, using $(\sqrt[3]2)^3 = 2$.) Is the quadratic factor irreducible over $K_1$? Its roots are exactly $\alpha_2, \alpha_3$ — both non-real. But $K_1 \subset \mathbb{R}$, so neither root lies in $K_1$, and a quadratic with no root in the field is irreducible over it. **This is precisely why one root was not enough**: the field generated by the real root is trapped inside $\mathbb{R}$ and cannot see the two complex roots. Adjoin a root of the quadratic, say $\alpha_2 = \omega\sqrt[3]2$: form $E = K_1(\alpha_2)$, with $[E : K_1] = 2$.

**Pass 3 — re-factor over $E$.** The quadratic now has one root $\alpha_2 \in E$; dividing it out leaves a linear factor, so the last root $\alpha_3$ is *automatically* in $E$ — no third adjunction needed. (Directly: by Vieta, $\alpha_2 + \alpha_3 = -\sqrt[3]2 \in E$ forces $\alpha_3 \in E$.) Now
$$x^3 - 2 = (x - \alpha_1)(x - \alpha_2)(x - \alpha_3) \quad \text{in } E[x]$$
— fully split. Done:
$$E = \mathbb{Q}(\alpha_1, \alpha_2) = \mathbb{Q}(\sqrt[3]2,\ \omega\sqrt[3]2) = \mathbb{Q}(\sqrt[3]2,\ \omega),$$
the last equality because $\omega = \alpha_2/\alpha_1 \in E$ and conversely $\alpha_2 = \omega\alpha_1$. Writing $E = \mathbb{Q}(\sqrt[3]2, \omega)$ is the tidiest form: one "radical" generator plus one "root of unity" generator. Degree bookkeeping by the Tower Law: $[E:\mathbb{Q}] = 2 \cdot 3 = 6 = 3!$ — at every pass the leftover factor stayed as irreducible as it possibly could, which is exactly when the $(\deg f)!$ bound is attained.

> **Remark 1 (the choices along the way don't matter).** In Pass 1 we chose the real root; adjoining $\omega\sqrt[3]2$ instead would give a *different* intermediate subfield of $\mathbb{C}$ (isomorphic to $\mathbb{Q}(\sqrt[3]2)$ but not equal to it — this is the "freedom of choice" in the uniqueness proof), yet the final splitting field comes out the same $E$. These different-but-isomorphic choices are exactly what the Galois group of the extension permutes (Section 9: $\operatorname{Gal}(E/\mathbb{Q}) \cong S_3$).

> **Remark 2 (why $\omega$ shows up — the general pattern for $x^n - a$).** The ratios of roots of $x^n - a$ are $n$-th roots of unity: if $\beta^n = \gamma^n = a$ then $(\beta/\gamma)^n = 1$. So the splitting field of $x^n - a$ over $\mathbb{Q}$ is always
> $$\mathbb{Q}(\sqrt[n]a,\ \zeta_n), \qquad \zeta_n = e^{2\pi i/n}$$
