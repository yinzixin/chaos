...
### A.1.5 The Maximal Ideal Criterion in Action

The Maximal Ideal Criterion (ring_field.md §13.2) says $M$ is maximal $\iff$ $R/M$ is a field, and its proof works by showing: for $a\notin M$, maximality forces $M+\langle a\rangle = R$, so $1 = m+ra$ for some $m\in M, r\in R$ — and reducing mod $M$ makes $r+M$ the inverse of $a+M$. We now watch this machinery run on actual numbers, in both directions, using $R=\mathbb{Z}$.

#### Direction 1: $M=5\mathbb{Z}$ is maximal $\implies \mathbb{Z}/5\mathbb{Z}$ is a field

*Maximality:* the only ideals of $\mathbb{Z}$ containing $5\mathbb{Z}$ are $d\mathbb{Z}$ for $d\mid 5$, i.e. $d\in\{1,5\}$ — giving only $\mathbb{Z}$ and $5\mathbb{Z}$ itself, since $5$ is prime. So no ideal sits strictly between $5\mathbb{Z}$ and $\mathbb{Z}$: $5\mathbb{Z}$ is maximal.

*Producing the inverse of $3+5\mathbb{Z}$, following the proof exactly:* take $a=3\notin 5\mathbb{Z}$. By maximality, $5\mathbb{Z}+\langle3\rangle$ can't be a proper ideal strictly containing $5\mathbb{Z}$, so it must equal $\mathbb{Z}$ — concretely, $\gcd(5,3)=1$ forces $5\mathbb{Z}+3\mathbb{Z}=\mathbb{Z}$ (§11). The extended Euclidean algorithm exhibits this:
$$5 = 1\cdot3+2, \quad 3=1\cdot2+1 \implies 1 = 3-1\cdot2 = 3-1\cdot(5-1\cdot3) = 2\cdot3 + (-1)\cdot5.$$
So $1 = (-5) + 2\cdot 3$, with $m=-5\in5\mathbb{Z}$ and $r=2$. Reducing mod $5\mathbb{Z}$: $1+5\mathbb{Z} = (2+5\mathbb{Z})(3+5\mathbb{Z})$ — so $(3+5\mathbb{Z})^{-1} = 2+5\mathbb{Z}$, i.e. $3^{-1}\equiv2\pmod5$. Check: $3\cdot2=6\equiv1$. $\checkmark$

*Repeating for every nonzero class* gives the full inverse table of $\mathbb{Z}_5^\times = \{1,2,3,4\}$:
$$1\cdot1\equiv1, \qquad 2\cdot3\equiv1, \qquad 3\cdot2\equiv1, \qquad 4\cdot4\equiv1 \pmod 5.$$
Every nonzero element has an inverse ($1,4$ self-inverse; $2,3$ mutually inverse) — exactly what "$\mathbb{Z}/5\mathbb{Z}=\mathbb{Z}_5$ is a field" means.

#### Direction 2 (why maximality is necessary): $M=4\mathbb{Z}$ is NOT maximal $\implies \mathbb{Z}/4\mathbb{Z}$ is NOT a field

*Non-maximality:* $4\mathbb{Z}\subsetneq2\mathbb{Z}\subsetneq\mathbb{Z}$ is a strictly increasing chain, so $4\mathbb{Z}$ is not maximal.

*Where the proof's construction breaks down:* take $a=2\notin4\mathbb{Z}$, and try to form $4\mathbb{Z}+\langle2\rangle$ as before. But $\gcd(4,2)=2\neq1$, so $4\mathbb{Z}+2\mathbb{Z}=2\mathbb{Z}$ (§11) — **not** all of $\mathbb{Z}$. Every element of $4\mathbb{Z}+2\mathbb{Z}=2\mathbb{Z}$ is even, so $1$ is simply unreachable: there is no $m\in4\mathbb{Z}, r\in\mathbb{Z}$ with $m+2r=1$. Consequently $2+4\mathbb{Z}$ has **no inverse** in $\mathbb{Z}_4$ — consistent with $2$ being a zero divisor there ($2\cdot2=4\equiv0$), so $\mathbb{Z}_4$ is not even an integral domain, let alone a field.

#### The mechanism, side by side

| | $M=5\mathbb{Z}$ (maximal) | $M=4\mathbb{Z}$ (not maximal) |
|---|---|---|
| Intermediate ideal? | none ($5$ prime) | $4\mathbb{Z}\subsetneq2\mathbb{Z}\subsetneq\mathbb{Z}$ |
| $a=$ | $3$ | $2$ |
| $M+\langle a\rangle$ | $5\mathbb{Z}+3\mathbb{Z}=\mathbb{Z}$ | $4\mathbb{Z}+2\mathbb{Z}=2\mathbb{Z}\neq\mathbb{Z}$ |
| Is $1$ reachable? | yes: $1=-5+2\cdot3$ | no: everything in $2\mathbb{Z}+4\mathbb{Z}$ is even |
| Inverse of $a+M$? | $2+5\mathbb{Z}$ | none — $2+4\mathbb{Z}$ is a zero divisor |
| $R/M$ | $\mathbb{Z}_5$, a field | $\mathbb{Z}_4$, not a field |

The entire criterion boils down to this single dichotomy: maximality of $M$ is exactly the condition that rules out the ideal sum $M+\langle a\rangle$ getting "stuck" at a proper intermediate ideal (as it does at $2\mathbb{Z}$ above) instead of reaching all of $R$ — and reaching $R$ is exactly what's needed to manufacture $1$, hence an inverse, for every nonzero coset.
...
#### Closing the gap: what does an *arbitrary* element generate?

The table above only lists ideals generated by an actual divisor of $n$. But $\mathbb{Z}_n$ has non-divisor elements too — e.g. $8 \in \mathbb{Z}_{12}$, and $8 \nmid 12$. What is $\langle 8\rangle$? The classification is only complete once we show that *every* element's ideal reduces to one on the list.

> **Theorem.** For any $d$ (thought of as an integer, reduced mod $n$), $\langle d\rangle = \langle \gcd(d,n)\rangle$ in $\mathbb{Z}_n$.

**Proof.** Let $g=\gcd(d,n)$. By Bézout's identity, $dx+ny=g$ for some integers $x,y$. Reducing mod $n$: $dx \equiv g \pmod n$, so $g \in \langle d\rangle$, giving $\langle g\rangle \subseteq \langle d\rangle$. Conversely, since $g \mid d$, write $d = g\cdot(d/g)$, so $d \in \langle g\rangle$, giving $\langle d\rangle \subseteq \langle g\rangle$. Hence $\langle d\rangle = \langle g\rangle$. $\square$

Since $\gcd(d,n)$ is always a divisor of $n$, this shows **every** element of $\mathbb{Z}_n$ generates one of the ideals already in the table above — an arbitrary generator always "collapses" onto the divisor it shares as a gcd with $n$. This is what makes the correspondence in the Fact above a complete classification, not just a list of examples.

**Corollary.** $\langle d\rangle = \mathbb{Z}_n \iff \gcd(d,n)=1$, i.e. iff $d$ is a unit (invertible element) of $\mathbb{Z}_n$ — *not* simply "$d \nmid n$".

**Example A3b (non-divisor generators).** In $\mathbb{Z}_{12}$: $\gcd(8,12)=4$, so $\langle 8\rangle = \langle 4\rangle = \{0,4,8\}$ — even though $8 \nmid 12$, it does *not* generate the whole ring. By contrast $\gcd(5,12)=1$, so $\langle 5\rangle = \langle 1\rangle = \mathbb{Z}_{12}$ — checking directly, $5,10,15\equiv3,20\equiv8,25\equiv1,30\equiv6,35\equiv11,40\equiv4,45\equiv9,50\equiv2,55\equiv7,60\equiv0$ visits all $12$ elements. The relevant condition is coprimality with $n$, not divisibility of $n$.
...
**Example A7.** In $\mathbb{Z}\oplus\mathbb{Z}$, take $I = 2\mathbb{Z}\oplus 3\mathbb{Z}$, so $R_1=R_2=\mathbb{Z}$, $I_1=2\mathbb{Z}$, $I_2=3\mathbb{Z}$. Then
...
#### What the elements of $R_1/I_1$ (and $R_2/I_2$) look like

Here $R_1/I_1 = \mathbb{Z}/2\mathbb{Z}$, whose elements are the two familiar infinite cosets from §A.1.3:
$$0+2\mathbb{Z} = \{\ldots,-4,-2,0,2,4,\ldots\} \ (\text{"evens"}), \qquad 1+2\mathbb{Z} = \{\ldots,-3,-1,1,3,5,\ldots\} \ (\text{"odds"}).$$
Likewise $R_2/I_2 = \mathbb{Z}/3\mathbb{Z}$ has three cosets, $0+3\mathbb{Z}$, $1+3\mathbb{Z}$, $2+3\mathbb{Z}$ (integers $\equiv 0,1,2 \bmod 3$).

An element of the quotient $(\mathbb{Z}\oplus\mathbb{Z})/(2\mathbb{Z}\oplus3\mathbb{Z})$ is a coset $(a,b) + (2\mathbb{Z}\oplus3\mathbb{Z})$, and by the Lemma this is exactly the *pair* $(a+2\mathbb{Z},\ b+3\mathbb{Z})$ — one coset from $R_1/I_1$ and one from $R_2/I_2$, matched up independently. So the whole quotient has $2\times3=6$ elements, all pairs of the above:
$$(0+2\mathbb{Z},\,0+3\mathbb{Z}),\ (0+2\mathbb{Z},\,1+3\mathbb{Z}),\ (0+2\mathbb{Z},\,2+3\mathbb{Z}),\ (1+2\mathbb{Z},\,0+3\mathbb{Z}),\ (1+2\mathbb{Z},\,1+3\mathbb{Z}),\ (1+2\mathbb{Z},\,2+3\mathbb{Z}).$$
Under the isomorphism to $\mathbb{Z}_2\oplus\mathbb{Z}_3$, these just become the $6$ pairs $(0,0),(0,1),(0,2),(1,0),(1,1),(1,2)$ — e.g. $(1+2\mathbb{Z},\,2+3\mathbb{Z})$ is the class of pairs $(a,b)\in\mathbb{Z}\oplus\mathbb{Z}$ with $a$ odd and $b\equiv2\pmod3$, matching the pair $(1,2)\in\mathbb{Z}_2\oplus\mathbb{Z}_3$. Note $\mathbb{Z}_2\oplus\mathbb{Z}_3$ is itself isomorphic to $\mathbb{Z}_6$ (Example 29, since $\gcd(2,3)=1$) — but that is a *second*, unrelated isomorphism internal to $\mathbb{Z}_2\oplus\mathbb{Z}_3$; the $6$ elements displayed above already are the full quotient $(\mathbb{Z}\oplus\mathbb{Z})/(2\mathbb{Z}\oplus3\mathbb{Z})$ without needing it.
